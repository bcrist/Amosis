...
const strings = struct {
    ...
    const product_name: descriptor.String("Amosis") = .{};
    const serial_number: descriptor.String("3") = .{};
};
...
pub const default_configuration = struct {
    ...
    pub const mouse_interface = struct {
        pub const index = 1;
        pub const class = usb.hid.class.boot_mouse;
        ...
        pub const hid_descriptor: usb.hid.boot_mouse.HID_Descriptor = .{};
        pub const report_descriptor: usb.hid.boot_mouse.Report_Descriptor = .{};

        pub const Report = usb.hid.boot_mouse.Input_Report;
    };
    ...
    //     pub const in_endpoint = struct {
    //         pub const address: endpoint.Address = .{ .ep = 4, .dir = .in };
    //         pub const kind: endpoint.Transfer_Kind = .bulk;
    //     };
    //     pub const out_endpoint = struct {
    //         pub const address: endpoint.Address = .{ .ep = 4, .dir = .out };
    //         pub const kind: endpoint.Transfer_Kind = .bulk;
    //     };
    ...
    pub const Descriptor_Set = packed struct {
        ...
        mouse_interface: descriptor.Interface = descriptor.Interface.parse(mouse_interface),
        mouse_hid: usb.hid.boot_mouse.HID_Descriptor = mouse_interface.hid_descriptor,
        mouse_hid_in_ep: descriptor.Endpoint = descriptor.Endpoint.parse(mouse_interface.in_endpoint),
...
pub fn get_interface_specific_descriptor(interface: u8, kind: descriptor.Kind, descriptor_index: u8) ?[]const u8 {
    ...
    switch (interface) {
        ...
        default_configuration.mouse_interface.index => {
            const mi = default_configuration.mouse_interface;
            switch (kind) {
                usb.hid.hid_descriptor => {
                    return mi.hid_descriptor.as_bytes();
                },
                usb.hid.report_descriptor => {
                    return mi.report_descriptor.as_bytes();
                },
                else => {},
            }
        },
...
pub fn is_endpoint_ready(address: endpoint.Address) bool {
    switch (address.dir) {
        .in => switch (address.ep) {
            ...
            default_configuration.mouse_interface.in_endpoint.address.ep => {
                return true;
            },
...
pub fn fill_in_buffer(ep: endpoint.Index, data: []u8) u16 {
    switch (ep) {
        ...
        default_configuration.mouse_interface.in_endpoint.address.ep => {
            const b = mouse_reporter.get_in_buffer();
            @memcpy(data.ptr, b);
            return @intCast(b.len);
        },
        // default_configuration.comm_interface.notification_endpoint.address.ep => {
...
pub fn handle_setup(setup: Setup_Packet) bool {
    ...
    if (mouse_reporter.handle_setup(setup)) return true;
    ...
    if (setup.kind == .class and setup.target == .interface) switch (setup.request) {
        usb.hid.requests.set_protocol => if (setup.direction == .out) {
            ...
            if (payload.interface == default_configuration.keyboard_interface.index or
                payload.interface == default_configuration.mouse_interface.index
            ) {
...
pub fn init() void {
    ...
    mouse_reporter = @TypeOf(mouse_reporter).init(&device);
...
pub var mouse_reporter: usb.hid.Callback_Input_Reporter(@This(), default_configuration.mouse_interface.Report, logic.get_mouse_report, .{
    .interface_index = default_configuration.mouse_interface.index,
    .report_id = 0,
}) = undefined;
